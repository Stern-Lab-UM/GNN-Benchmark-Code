function manifest = DCG_run_publication_pipeline(varargin)
% DCG_run_publication_pipeline  MATLAB top-level DCG/GNN publication pipeline.
%
%   MANIFEST = DCG_run_publication_pipeline('mode','mini') runs a small but
%   trainable end-to-end check. MANIFEST =
%   DCG_run_publication_pipeline('mode','publication') uses the same stages at
...
fprintf('\n============================================================\n');
fprintf('[DCG pipeline] mode:        %s\n', opts.mode);
fprintf('[DCG pipeline] output root: %s\n', paths.output_root);
fprintf('[DCG pipeline] repo root:   %s\n', paths.repo_root);
fprintf('============================================================\n\n');
...
for i = 1:numel(stages)
    ...
    try
        switch stage
            ...
            otherwise
                error('DCG:pipeline:badStage', 'Unknown stage "%s".', stage);
        end
        secs = toc(t0);
        ...
        fprintf(2, '[DCG pipeline] FAILED in %s after %.1f s:\n%s\n', ...
...
fprintf('\n[DCG pipeline] DONE. Manifest:\n  %s\n', fullfile(paths.manifests, 'pipeline_manifest.json'));
end

function opts = parse_inputs(varargin)
p = inputParser;
p.FunctionName = 'DCG_run_publication_pipeline';
...
p.addParameter('python', getenv_or('DCG_PYTHON', 'python'), @(x) ischar(x) || isstring(x));
...
for j = 1:numel(jobs)
    ...
    if strcmp(job.family, 'MPNN')
        ...
        ppgn = stage_ppgn_dataset(job, paths, 'train_dcg');
        if opts.run_bayesopt
            results = optimize_PPGN(ppgn.dataset_file, job.split_dir, search.hp_ranges, search.n_trials, ...
                ...
                'dcg_cmd', [opts.python, ' -m dcg.main']);
            hp = best_hps_from_bayesopt(results, search.hp_ranges, job.family);
        else
            hp = default_hps(job.model, job.family);
        end
...
for j = 1:numel(jobs)
    ...
    for s = opts.seeds
        ...
        if strcmp(job.family, 'MPNN')
            ...
            [status, output] = run_logged(cmd, log_file, paths.repo_root);
            if status ~= 0, error('DCG:pipeline:mpnnTrain', 'MPNN training failed: %s\n%s', log_file, output); end
            ...
            [status, output] = run_logged(cmd, log_file, paths.repo_root);
            if status ~= 0, error('DCG:pipeline:ppgnTrain', 'PPGN training failed: %s\n%s', log_file, output); end
        end
...
for j = 1:numel(jobs)
    ...
    for s = opts.seeds
        ...
        if strcmp(job.family, 'MPNN')
            ...
            ppgn = stage_ppgn_dataset(job, paths, 'predict_dcg');
            model_dir = fullfile(paths.model_root, job.job_id, sprintf('seed_%d', s));
            cmd = ppgn_predict_cmd(opts, model_dir, ppgn.dataset_file, raw_out);
        end
        [status, output] = run_logged(cmd, log_file, paths.repo_root);
        if status ~= 0, error('DCG:pipeline:predict', 'Prediction failed: %s\n%s', log_file, output); end
...
if isempty(opts.embedding_root)
    out.message = ['No embedding_root supplied. Embedding example panels can be generated by ', ...
        'the figure scripts when DCG_EMBED_ENGINE and vt2d paths are configured.'];
    fprintf('[embedding] %s\n', out.message);
    return
end
out.result = DCG_analyze_embedding_error_bounds('embedding_root', opts.embedding_root, ...
...
if strcmp(opts.mode, 'mini')
    ...
    run(fullfile(paths.repo_root, 'analysis', 'matlab', 'DCG_rebuild_all_summaries.m'));
...
if strcmp(opts.mode, 'mini')
    ...
    data_root = ternary(isempty(opts.data_root_for_figures), paths.pred_consolidated_root, opts.data_root_for_figures);
    assignin('base', 'data_root', data_root);
    run(fullfile(paths.repo_root, 'analysis', 'matlab', 'DCG_plot_everything.m'));
    out = struct('status', 'ok', 'data_root', data_root);
end
end
...
for d = 1:numel(opts.datasets)
    ...
    if ~isfolder(ds_dir), error('DCG:pipeline:missingDataset', 'Missing %s', ds_dir); end
...
if isempty(split_dirs), error('DCG:pipeline:noSplits', 'No split folders for %s', ds_dir); end
end
...
if ~isfile(file), error('DCG:pipeline:missingDataFile', 'Missing %s', file); end
end
...
src = fullfile(repo_root(), 'models', 'ppgn', 'train_dcg');
args = ppgn_args(job, hp, opts.final_epochs);
core = sprintf('"%s" -m dcg.main train --training-data "%s" --out-dir "%s" --inds-dir "%s" --no-evaluation --args "%s"', ...
...
src = fullfile(repo_root(), 'models', 'ppgn', 'predict_dcg');
core = sprintf('"%s" -m dcg.main predict --model "%s" --input "%s" --output "%s"', opts.python, model_dir, data_file, out_file);
cmd = with_pythonpath(core, src, opts.cuda);
end
...
if isempty(files), error('DCG:pipeline:noPredictions', 'No predictions in %s', paths.pred_raw_root); end
...
error('DCG:pipeline:noCheckpoint', 'No checkpoint found under %s', root);
end
