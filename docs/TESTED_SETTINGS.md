...
| 2026-06-27 | Full default mini pipeline validation after deterministic PPGN mini stabilizer | Lighthouse `lh0500` A100 80GB PCIe | MATLAB R2024b; Python 3.11.13 venv; PyTorch 2.12.1+cu130 | `GNNBenchmark_run_publication_pipeline('mode','mini','cuda',0,...)` via `_remote_q_lh/codex_lh_ppgn_mini_fix_validate_20260627_004` | Passed end-to-end through BO, training, prediction, embedding, analysis, and figure creation. PPGN mini BO still ran, final mini PPGN training used fixed smoke-test HPs, and the MAE sanity table had max MAE = 2.1668 (PPGN-UW), below the threshold of 10. |
| 2026-06-28 | Fresh-clone reduced integration validation from repository bundle | Lighthouse `lh0500` A100 80GB PCIe | MATLAB R2024b; Python 3.11.13 venv; PyTorch 2.12.1+cu130; PyTorch Geometric 2.8.0 | Fresh source bundle at commit `3555725`; `GNNBenchmark_run_publication_pipeline('mode','integration','prompt_cache',false,'cache_policy',all false,...)` via `_remote_q_lh/codex_lh_de_novo_integration_full_3555725_20260628_001` | Passed end-to-end through install check, data generation, Bayesian optimization, final training, prediction, spring embedding, summary rebuilding, and all manuscript/revision figure-generation wrappers. The run produced 206 consolidated prediction files, 372 per-graph embedding files, and 96 figure files across the canonical figure folders. No failed stage, no missing-vt2d embedding warning, and no remaining vt2d lookup warning were present in the final log. |
