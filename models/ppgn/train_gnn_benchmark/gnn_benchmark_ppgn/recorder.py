"""Utilities for models / ppgn / train_gnn_benchmark / gnn_benchmark_ppgn / recorder.py in the GNN Benchmark codebase."""
...
class Recorder():
    ...
    def load_model_state(self, config: Configuration):
        ...
        with self.get_out_file(self.model_file, zipped=True, mode='r') as file:
            # weights_only=False because gnn_benchmark_ppgn's checkpoint includes a pandas
            # DataFrame (norm_factors), which PyTorch>=2.6 refuses to load
            # under the new safe-default. The .tar files are produced by
            # the same gnn_benchmark_ppgn pipeline, so this is trusted.
            saved_states = torch.load(file, map_location=DEVICE,
                                      weights_only=False)
...
class SavedModel():
    """
    Provide the saved model component used by models / ppgn / train_gnn_benchmark / gnn_benchmark_ppgn / recorder.py.
