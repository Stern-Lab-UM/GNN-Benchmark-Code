"""Utilities for models / ppgn / predict_gnn_benchmark / gnn_benchmark_ppgn / file_reader.py in the GNN Benchmark codebase."""
...
class FileReader():
    """
    Coordinate file reader responsibilities for the gnn_benchmark_ppgn PPGN workflow.
...
#                        graphs = np.empty(self.num_graphs, dtype=np.object)
#                        targets = np.empty(self.num_graphs, dtype=np.object)
                    if self.num_cells is None:
                        # when there are multiple sizes in file
                        graphs  = np.empty(self.num_graphs, dtype=object)
                        targets = np.empty(self.num_graphs, dtype=object)
                        # ensure predictable "is None" checks later
                        graphs[:]  = None
                        targets[:] = None
                    else:
                        # one size in file
        ...
        graph_num += 1  # convert to 1-based, is this necessary?
...
#                             f' expected {self.num_cells}')
    ...
    def _parse_binary(self, data):
        """
        Implement the parse binary step for models / ppgn / predict_gnn_benchmark / gnn_benchmark_ppgn / file_reader.py.
...
class CellParser():
    """
    Coordinate cell parser responsibilities for the gnn_benchmark_ppgn PPGN workflow.
...
class NamedTokens():
    ...
    def inds(self):
        """
        Implement the inds step for models / ppgn / predict_gnn_benchmark / gnn_benchmark_ppgn / file_reader.py.
    ...
    def names(self):
        """
        Implement the names step for models / ppgn / predict_gnn_benchmark / gnn_benchmark_ppgn / file_reader.py.
    ...
    def toks(self):
        """
        Implement the toks step for models / ppgn / predict_gnn_benchmark / gnn_benchmark_ppgn / file_reader.py.
...
class NodeMap(dict):
    """
    Provide the node map component used by models / ppgn / predict_gnn_benchmark / gnn_benchmark_ppgn / file_reader.py.
