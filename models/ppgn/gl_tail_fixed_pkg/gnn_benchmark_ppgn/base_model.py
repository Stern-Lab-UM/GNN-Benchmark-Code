"""Utilities for models / ppgn / gl_tail_fixed_pkg / gnn_benchmark_ppgn / base_model.py in the GNN Benchmark codebase."""
...
class BaseModel(nn.Module):
    """
    Coordinate base model responsibilities for the gnn_benchmark_ppgn PPGN workflow.
    ...
    def predict(self, x):
        ...
        Returns:
            Computed value used by the caller.
        """
        # FIX: route the trained-model inference through forward() so predict
        # and training produce identical scores. The previous implementation
        # mixed denorm into the mask/symm steps and disagreed with forward()
        # at every edge. Input z-score and output denorm bracket the call —
        # the loader's predict path runs with apply_norm=False, so the input
        # normalization MUST happen here (for IN=true configs in_means/in_stds
        # are identity and this becomes a no-op; for IN=false configs the
        # global z-score is essential).
        self.eval()
        with torch.no_grad():
            x_norm = ((x - self.in_means[None, :, None, None]) /
                      self.in_stds[None, :, None, None] *
                      self.input_mask(x)).to(torch.float)
            scores = self.forward(x_norm)
            return (scores * self.out_stds[None, :, None, None] +
                    self.out_means[None, :, None, None])

    def output_mask(self, x):
        """
        Implement the output mask step for models / ppgn / gl_tail_fixed_pkg / gnn_benchmark_ppgn / base_model.py.
    ...
    def input_mask(self, x):
        """
        Implement the input mask step for models / ppgn / gl_tail_fixed_pkg / gnn_benchmark_ppgn / base_model.py.
    ...
    def _find_in_targets(self, args, targets):
        """
        Implement the find in targets step for models / ppgn / gl_tail_fixed_pkg / gnn_benchmark_ppgn / base_model.py.
    ...
    def initialize_from(self, other):
        """
        Implement the initialize from step for models / ppgn / gl_tail_fixed_pkg / gnn_benchmark_ppgn / base_model.py.
