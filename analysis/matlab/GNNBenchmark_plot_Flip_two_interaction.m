function outputs = GNNBenchmark_plot_Flip_two_interaction(analyses_filename, figures_output_dir, save_png, single_t1_analyses_filename)
% GNNBenchmark_plot_Flip_two_interaction  Implement GNN Benchmark plot flip two interaction for this MATLAB workflow.
% Inputs: analyses_filename, figures_output_dir, save_png, single_t1_analyses_filename
% Outputs: outputs
%GNNBenchmark_PLOT_FLIP_TWO_INTERACTION
...
% ============================ FILE OVERVIEW ============================
% CONTEXT
%   Part of the GNN Benchmark analysis pipeline (npj revision). The
...
%   second flip? The standard GNN Benchmark plotter only knows the distance to the
...
%   * Defaults are resolved from GNNBenchmark_publication_config; pass explicit paths to override.
...
fprintf('[GNNBenchmark_plot_Flip_two_interaction] loading %s\n', analyses_filename);
...
fprintf('[GNNBenchmark_plot_Flip_two_interaction] loading single-T1 reference %s\n', single_t1_analyses_filename);
...
fprintf('[GNNBenchmark_plot_Flip_two_interaction] complete. Output: %s\n', figures_output_dir);
...
%     GNNBenchmark_analyze_results; +Inf means unreachable and is filtered.
...
fprintf('[GNNBenchmark_plot_Flip_two_interaction] extracted %d graphs, %d nearest-hop records, %d distance-pair records, %d zone records.\n', ...
...
fprintf(['[GNNBenchmark_plot_Flip_two_interaction] extracted single-T1 reference ', ...
...
if isempty(candidates)
    error('GNNBenchmark:missingSingle16CohortSubset', ...
...
if numel(candidates) > 1
    error('GNNBenchmark:ambiguousSingle16CohortSubset', ...
...
for s = 1 : n_seeds
    if size(T.inds, 1) < subset_i || size(T.inds, 2) < s || isempty(T.inds{subset_i,s}) || ...
        ...
        error('GNNBenchmark:missingSingle16CohortSplit', ...
...
if any(n_test == 0)
    error('GNNBenchmark:emptySingle16CohortSplit', ...
...
% pair_key  Implement pair key for analysis/matlab/GNNBenchmark_plot_Flip_two_interaction.m.
% Inputs: a, b
% Outputs: key
        if a > b
            tmp = a;
...
% rows_for_pair  Implement rows for pair for analysis/matlab/GNNBenchmark_plot_Flip_two_interaction.m.
% Inputs: a, b
% Outputs: rows
        key = pair_key(a, b);
        if isKey(pairRows, key)
...
% connect_sets  Implement connect sets for analysis/matlab/GNNBenchmark_plot_Flip_two_interaction.m.
% Inputs: rowsA, rowsB
...
if any(root_rows < 1) || any(root_rows > numnodes(line_G))
    error('GNNBenchmark:FlipTwoRootOutOfRange', ...
...
if ~isequal(expected, actual)
    error('GNNBenchmark:FlipTwoRootRowMismatch', ...
...
if ~isempty(missing)
    warning('GNNBenchmark:missingPairVsSingleModel', ...
...
%     match GNNBenchmark_plot_results. So log_seed = log2(mean(error)) (log
...
%   (matching GNNBenchmark_analyze_results), d1/d2/d_near/d_far and inter-flip
...
GNNBenchmark_savefig_visible(fig, output_path);
