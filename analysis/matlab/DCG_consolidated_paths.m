...
switch action
    ...
    otherwise
        error('DCG_consolidated_paths:badAction', 'unknown action "%s"', action);
end
end
...
%   ERRORS   throws DCG_consolidated_paths:prefix if none of the schemes match.
...
if ~isempty(tok)
    ...
    error('DCG_consolidated_paths:prefix', 'unrecognized rev prefix "%s"', prefix);
end
...
error('DCG_consolidated_paths:prefix', 'unrecognized prefix "%s"', prefix);
end
