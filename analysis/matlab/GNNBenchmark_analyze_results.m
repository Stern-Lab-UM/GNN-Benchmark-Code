...
%   GNNBenchmark_consolidated_paths and read_dataset_inds.
%
...
%
% HOW TO RUN
%   Set dataset directly, or pass a GNNBenchmark_CONFIG struct from a driver script.
%   Set data_root through GNNBenchmark_CONFIG.data_root, the GNN_BENCHMARK_DATA_ROOT environment
%   variable, or an untracked GNNBenchmark_local_config.m file.
%
%   After this script finishes, use GNNBenchmark_plot_results or GNNBenchmark_plot_everything to
...
% The struct calling convention (GNNBenchmark_run_revision_analyses.m) passes
% the dataset via GNNBenchmark_CONFIG.dataset; honor it BEFORE the default and the
...
if exist('GNNBenchmark_CONFIG', 'var') && isstruct(GNNBenchmark_CONFIG) ...
        && isfield(GNNBenchmark_CONFIG, 'dataset') && ~isempty(GNNBenchmark_CONFIG.dataset)
...
% Optional caller override. GNNBenchmark_run_all_datasets.m can pre-set `dataset`;
% callers may also pre-set GNNBenchmark_CONFIG with any of these fields:
...
if exist('GNNBenchmark_CONFIG', 'var') && isstruct(GNNBenchmark_CONFIG)
    ...
    if isfield(GNNBenchmark_CONFIG, 'inds_root'), inds_root = GNNBenchmark_CONFIG.inds_root; end
    if isfield(GNNBenchmark_CONFIG, 'seeds'), seeds = GNNBenchmark_CONFIG.seeds; end
    if isfield(GNNBenchmark_CONFIG, 'MP_models'), MP_models = GNNBenchmark_CONFIG.MP_models; end
    if isfield(GNNBenchmark_CONFIG, 'data_sets'), data_sets = GNNBenchmark_CONFIG.data_sets; end
    if isfield(GNNBenchmark_CONFIG, 'load_precomputed_data'), load_precomputed_data = GNNBenchmark_CONFIG.load_precomputed_data; end
    if isfield(GNNBenchmark_CONFIG, 'add_node_features'), add_node_features = GNNBenchmark_CONFIG.add_node_features; end
    if isfield(GNNBenchmark_CONFIG, 'cache_dir'), cache_dir = GNNBenchmark_CONFIG.cache_dir; end
    if isfield(GNNBenchmark_CONFIG, 'output_filename'), output_filename = GNNBenchmark_CONFIG.output_filename; end
    ...
    if isfield(GNNBenchmark_CONFIG, 'is_new_interface_extra')
        is_new_interface_extra = GNNBenchmark_CONFIG.is_new_interface_extra;
    end
end
...
is_consolidated = GNNBenchmark_consolidated_paths('is_consolidated', data_root);
if is_consolidated
...
if has_ppgn
    ...
    for i = 1 : numel(ppgn_expected_prefixes)
        if is_consolidated
            glob_path = GNNBenchmark_consolidated_paths('pred_glob', ...
...
fprintf('[GNNBenchmark_analyze_results] dataset=%s | tasks=%s | has_ppgn=%d | n_cells=%d\n', ...
    dataset, strjoin(tasks, ','), has_ppgn, n_cells);
if ~isempty(ppgn_expected_prefixes)
    fprintf('[GNNBenchmark_analyze_results] PPGN prefixes matched: %d/%d\n', ...
        ...
        for i = 1 : size(I.(all_models{m}).(tasks{t}).vals,1)
            ...
            for s = 1 : length(seeds)
                try
                    ...
                    error('GNNBenchmark:filterIndex', ...
            ...
            for m = 2 : length(all_models)
                if t == 1
                    try
                        ...
                        error('GNNBenchmark:disabledOrderingCheckFailed', 'Historical ordering check failed.');
                    end
                else
                    try
    ...
    for s = 1 : length(seeds)
        ...
        for ss = 1 : length(curr_dataset)
            ...
            try
                ...
                error('GNNBenchmark:graphPlaceholderAccess', ...
            ...
            line_G = GNNBenchmark_line_graph_preserve_rows(curr_G(:,1:2));
            ...
            if numel(root_edge_in_line_G) ~= expected_root_count
                error('GNNBenchmark:unexpectedT1RootCount', ...
            ...
            GNNBenchmark_assert_root_rows_match_line_graph(line_G, curr_G(:,1:2), root_edge_in_line_G, ...
            ...
            if ismember('none_to_lengths', tasks)
                ...
                try
                    ...
                    if ~isequal(S.ground_truth.none_to_lengths{curr_idx, curr_col, s}.(data_sets{1}){j}(to_compare1), curr_ground_truth_none_to_lengths(to_compare1,1)) || ~isequal(S.ground_truth.lengths_to_lengths{curr_idx, curr_col, s}.(data_sets{1}){j}(to_compare2), curr_ground_truth_lengths_to_lengths(to_compare2,1))
                        error('GNNBenchmark:groundTruthMismatch', ...
                            'Ground-truth columns disagree across tasks/models (subset=%d, seed=%d, graph=%d).', ...
                            i, s, j);
                    end
                catch ME
                    error('GNNBenchmark:groundTruthCheckFailed', ...
            ...
            if n_bad_nbhd > 0 && ~strcmp(dataset, 'kA_1')
                warning('GNNBenchmark:edgeHexNeighborhood', ...
...
source_manifest = GNNBenchmark_source_manifest(data_root); %#ok<NASGU>
...
consolidated = GNNBenchmark_consolidated_paths('is_consolidated', data_root);
...
for t = 1 : length(tasks)
    ...
    parfor k = 1 : N
        ...
        if consolidated
            curr_filename = GNNBenchmark_consolidated_paths('pred_file', ...
    ...
    if ~isequal(n_graphs, loaded_graphs)
        error('GNNBenchmark:graphCountMismatch', ...
...
for t = 1 : length(tasks)
    ...
    if any(loaded_mask)
        ...
        if ~isequal(n_graphs_hdr(:), n_graphs_got(:))
            warning('GNNBenchmark:ppgnGraphCount', ...
...
if ~isempty(data_root) && GNNBenchmark_consolidated_paths('is_consolidated', data_root)
    % Consolidated snapshot: split is in splits\<key>\, chosen by _applies_to.
    inds_dir = GNNBenchmark_consolidated_paths('inds_dir', data_root, prefix);
    if isempty(inds_dir)
...
function LineG = GNNBenchmark_line_graph_preserve_rows(C, varargin)
% GNNBenchmark_line_graph_preserve_rows  Build a line graph whose rows preserve source edge ordering.
% Inputs: C, varargin
% Outputs: LineG
%GNNBenchmark_LINE_GRAPH_PRESERVE_ROWS
...
% pair_key  Implement pair key for analysis/matlab/GNNBenchmark_analyze_results.m.
% Inputs: a, b
% Outputs: key
        if a > b
            tmp = a;
...
% rows_for_pair  Implement rows for pair for analysis/matlab/GNNBenchmark_analyze_results.m.
% Inputs: a, b
% Outputs: rows
        key = pair_key(a, b);
        if isKey(pairRows, key)
...
% connect_sets  Implement connect sets for analysis/matlab/GNNBenchmark_analyze_results.m.
% Inputs: rowsA, rowsB
...
function GNNBenchmark_assert_root_rows_match_line_graph(line_G, original_edges, root_rows, context)
% GNNBenchmark_assert_root_rows_match_line_graph  Build a line graph whose rows preserve source edge ordering.
% Inputs: line_G, original_edges, root_rows, context
% Outputs: none; performs side effects or updates the caller workflow.
%GNNBenchmark_ASSERT_ROOT_ROWS_MATCH_LINE_GRAPH
% Fail if line-graph node rows do not match original prediction rows.

if any(root_rows < 1) || any(root_rows > numnodes(line_G))
    error('GNNBenchmark:rootOutOfLineGraphRange', ...
...
if ~isequal(expected, actual)
    error('GNNBenchmark:rootLineGraphRowMismatch', ...
