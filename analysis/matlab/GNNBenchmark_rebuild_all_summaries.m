%==========================================================================
%  GNNBenchmark_rebuild_all_summaries  --  Step-1 rebuild wrapper (all 10)
%
%  PURPOSE
...
%    permanent, committed equivalent of the ad-hoc %TEMP%\GNNBenchmark_rebuild_all10.m
%    used on 2026-06-01.
%
%  WHY THIS EXISTS
%    GNNBenchmark_run_revision_analyses's DEFAULT dataset list omits `hex`
...
%
%  USAGE
%    >> run('GNNBenchmark_rebuild_all_summaries.m')
%    Then run GNNBenchmark_plot_everything.m for the figures.
%
%  NOTE
...
plot_after_summary = false;    % summaries only; figures come from GNNBenchmark_plot_everything

code_dir = fileparts(mfilename('fullpath'));
fprintf('[GNNBenchmark_rebuild_all_summaries] rebuilding %d datasets (no plots)...\n', numel(datasets));
run(fullfile(code_dir, 'GNNBenchmark_run_revision_analyses.m'));
fprintf('[GNNBenchmark_rebuild_all_summaries] done. Run GNNBenchmark_plot_everything.m for figures.\n');
