function outputs = GNNBenchmark_analyze_embedding_error_bounds(varargin)
%GNNBenchmark_ANALYZE_EMBEDDING_ERROR_BOUNDS  Relate graph prediction error to embedding error.
%
%   OUTPUTS = GNNBenchmark_ANALYZE_EMBEDDING_ERROR_BOUNDS(...) scans saved per-graph
...
%                          <GNN_BENCHMARK_DATA_ROOT>/embeddings/per_graph when
%                          GNN_BENCHMARK_DATA_ROOT is configured.
...
p = inputParser;
p.FunctionName = 'GNNBenchmark_analyze_embedding_error_bounds';
...
if isempty(opts.embedding_root) || ~isfolder(opts.embedding_root)
    error('GNNBenchmark:embeddingRootMissing', ['Set embedding_root or GNN_BENCHMARK_DATA_ROOT. ', ...
...
if isempty(files)
    error('GNNBenchmark:noEmbeddingFiles', 'No files matching %s under %s.', opts.file_glob, opts.embedding_root);
end
...
for i = 1:numel(files)
    ...
    if isempty(M) || size(M, 2) < max_col
        warning('GNNBenchmark:badEmbeddingFile', 'Skipping unreadable/short embedding file: %s', files{i});
        continue;
    end
    ...
    if ~any(ok)
        warning('GNNBenchmark:emptyEmbeddingFile', 'Skipping file with no finite length triplets: %s', files{i});
        continue;
    end
...
rows = rows(1:n);
if isempty(rows)
    error('GNNBenchmark:noUsableEmbeddingFiles', 'No usable embedding files were found under %s.', opts.embedding_root);
end
...
fprintf('[GNNBenchmark_analyze_embedding_error_bounds] files read: %d usable / %d scanned\n', height(per_graph), numel(files));
fprintf('[GNNBenchmark_analyze_embedding_error_bounds] wrote %s\n', opts.output_dir);
end
