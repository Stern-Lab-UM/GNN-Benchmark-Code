function output_paths = GNNBenchmark_make_revision_transverse_dist_composites(figures_root, save_png)
% GNNBenchmark_make_revision_transverse_dist_composites  Implement GNN Benchmark make revision transverse dist composites for this MATLAB workflow.
% Inputs: figures_root, save_png
% Outputs: output_paths
%GNNBenchmark_MAKE_REVISION_TRANSVERSE_DIST_COMPOSITES
...
% npj revision of the GNN Benchmark study. It does NOT recompute any
...
%   GNNBenchmark_make_revision_transverse_dist_composites - main driver
...
%   GNNBenchmark_savefig_visible   - make figure visible, name it, save as .fig
...
%
% ---------------------------------------------------------------------------
% FUNCTION: GNNBenchmark_make_revision_transverse_dist_composites (main)
% ---------------------------------------------------------------------------
...
%        e. Save the composite via GNNBenchmark_savefig_visible; if save_png, also export
...
for f = 1 : size(figure_names, 1)
    ...
    GNNBenchmark_savefig_visible(fig, output_paths{f});
    if save_png
        png_path = regexprep(output_paths{f}, '\.fig$', '.png');
        exportgraphics(fig, png_path, 'Resolution', 300);
    end
    fprintf('[GNNBenchmark_make_revision_transverse_dist_composites] saved %s\n', output_paths{f});
end
...
% Outputs: none; performs side effects or updates the caller workflow.
% ---------------------------------------------------------------------------
% FUNCTION: GNNBenchmark_savefig_visible (local)
% ---------------------------------------------------------------------------
