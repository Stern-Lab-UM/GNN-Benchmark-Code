function outputs = DCG_analyze_counterfactual_copying(varargin)
%DCG_ANALYZE_COUNTERFACTUAL_COPYING  Quantify distal input-copy behavior.
%
%   OUTPUTS = DCG_ANALYZE_COUNTERFACTUAL_COPYING(...) compares regular
...
p = inputParser;
p.FunctionName = 'DCG_analyze_counterfactual_copying';
...
if isempty(opts.regular_pred_root) || ~isfolder(opts.regular_pred_root)
    error('DCG:missingRegularRoot', 'regular_pred_root is missing or not a folder.');
end
if isempty(opts.counterfactual_pred_root) || ~isfolder(opts.counterfactual_pred_root)
    error('DCG:missingCounterfactualRoot', 'counterfactual_pred_root is missing or not a folder.');
end
...
if isempty(test_indices)
    warning('DCG:noTestInds', 'No inds_dir/test.inds supplied; analyzing all graphs in each prediction file.');
end
...
if isempty(pairs)
    error('DCG:noPredictionPairs', 'No regular/counterfactual prediction-file pairs were found. Adjust root/token options.');
end
...
for pidx = 1:height(pairs)
    ...
    for gi = reshape(graph_ids, 1, [])
        ...
        if isempty(cf_gi)
            warning('DCG:missingGraph', 'Counterfactual file lacks graph %s; skipping.', gname);
            continue;
        end
        ...
        if isempty(R) || size(R,2) < opts.pred_col || size(C,2) < opts.pred_col
            warning('DCG:badPredictionGraph', 'Skipping graph with short/misaligned rows: %s', gname);
            continue;
        end
...
fprintf('[DCG_analyze_counterfactual_copying] paired files: %d\n', height(pairs));
fprintf('[DCG_analyze_counterfactual_copying] wrote %s\n', opts.output_dir);
end
...
for m = 1:numel(models)
    ...
    for seed = reshape(seeds, 1, [])
        ...
        if numel(reg) > 1 || numel(cf) > 1
            error('DCG:ambiguousPredictionPair', ['Ambiguous files for %s seed %d. ', ...
...
if ~isfile(f)
    error('DCG:testIndsMissing', 'test.inds not found in %s.', inds_dir);
end
...
if isempty(test_indices)
    ...
    if max(test_indices) > numel(reg_names)
        error('DCG:testIndexOutOfRange', 'test.inds contains index %d but %s has %d graphs.', ...
...
if ~isempty(missing)
    error('DCG:graphNameMismatch', 'Counterfactual file is missing %d selected graph names, first: %s', ...
