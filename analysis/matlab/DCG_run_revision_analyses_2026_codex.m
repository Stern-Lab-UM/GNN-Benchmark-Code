%==========================================================================
% DCG_run_revision_analyses_2026_codex
%
...
% then runs DCG_plot_Flip_two_interaction_2026_codex, which produces the
...
analyzer_script = fullfile(code_dir, 'DCG_analyze_results_2026_codex.m');
plotter_script = fullfile(code_dir, 'DCG_plot_results_2026_codex.m');
calibrator_function = fullfile(code_dir, 'DCG_calibrate_saved_fig_y_ranges_2026_codex.m');
composite_function = fullfile(code_dir, 'DCG_make_revision_transverse_dist_composites_2026_codex.m');
flip_two_interaction_function = fullfile(code_dir, 'DCG_plot_Flip_two_interaction_2026_codex.m');
flip_two_verify_function = fullfile(code_dir, 'DCG_verify_Flip_two_figures_2026_codex.m');

if ~exist('data_root', 'var') || isempty(data_root)
    consolidated_pc  = 'Z:\Tomer\gnn_benchmark_consolidated_20260530';
    consolidated_nfs = '/nfs/turbo/dent-tomers/Tomer/gnn_benchmark_consolidated_20260530';
    if ispc && isfolder(consolidated_pc)
        data_root = consolidated_pc;
    elseif ~ispc && isfolder(consolidated_nfs)
        data_root = consolidated_nfs;
    else
        error('DCG:noConsolidated', ['Consolidated snapshot not found at %s (PC) / %s (cluster). ', ...
            'Set data_root explicitly. The old Desktop and DCG-2024 trees are RETIRED and must not be used.'], ...
            consolidated_pc, consolidated_nfs);
    end
end

is_consolidated_root = DCG_consolidated_paths_2026_codex('is_consolidated', data_root);
if is_consolidated_root
    % Snapshot has no legacy analyses_data.mat; parse it fresh into a codex
    % cache + figures tree kept inside the snapshot folder.
    source_cache_root = fullfile(data_root, '_analyzer_cache');
    codex_cache_root  = fullfile(source_cache_root, 'revision_codex_2026');
    figures_root      = fullfile(data_root, '_figures', 'revision_codex_2026');
elseif endsWith(data_root, 'GNN benchmark results')
    source_cache_root = fullfile(data_root, '_analyzer_cache');
    codex_cache_root = source_cache_root;
    figures_root = fullfile(data_root, '_figures');
else
    source_cache_root = fullfile(data_root, 'Analyzer cache');
    codex_cache_root = fullfile(source_cache_root, 'revision_codex_2026');
    figures_root = fullfile(data_root, 'Output figures', 'revision_codex_2026');
end

if ~isfolder(codex_cache_root), mkdir(codex_cache_root); end
if ~isfolder(figures_root), mkdir(figures_root); end

fprintf('[DCG_run_revision_analyses_2026_codex] data root:    %s\n', data_root);
fprintf('[DCG_run_revision_analyses_2026_codex] source cache: %s\n', source_cache_root);
fprintf('[DCG_run_revision_analyses_2026_codex] codex cache:  %s\n', codex_cache_root);
fprintf('[DCG_run_revision_analyses_2026_codex] figures:      %s\n', figures_root);
fprintf('[DCG_run_revision_analyses_2026_codex] excluded models: %s\n', strjoin(models_to_exclude, ', '));
...
for batch_dataset_index_2026 = 1 : numel(datasets)
    ...
    codex_summary = fullfile(codex_cache_root, [source_dataset, ' - results_summary.mat']);
    need_summary = rebuild_summaries || ~isfile(codex_summary);
    ...
    if is_consolidated_root
        ...
        if need_summary
            codex_analysis = fullfile(codex_cache_root, [source_dataset, ' - analyses data.mat']);
            fprintf('Building summary from snapshot: %s\n', data_root);
            DCG_CONFIG = struct();
            DCG_CONFIG.dataset = source_dataset;
            DCG_CONFIG.data_root = data_root;
            DCG_CONFIG.load_precomputed_data = ~rebuild_summaries && isfile(codex_analysis);
            DCG_CONFIG.output_filename = codex_analysis;
            DCG_CONFIG.results_summary_filename = codex_summary;
            DCG_CONFIG.cache_dir = codex_cache_root;
            run(analyzer_script);
            clear DCG_CONFIG;
        else
            fprintf('Using existing codex summary: %s\n', codex_summary);
        end
    else
...
if plot_after_summary && calibrate_y_ranges && isfile(calibrator_function)
    DCG_calibrate_saved_fig_y_ranges_2026_codex(figures_root, datasets, [], save_png);
end

if plot_after_summary && make_composite_figures && isfile(composite_function)
    DCG_make_revision_transverse_dist_composites_2026_codex(figures_root, save_png);
end
...
if plot_after_summary && make_flip_two_interaction_figures && any(strcmp(datasets, 'Flip_two')) && isfile(flip_two_interaction_function)
    % Prefer the FRESH analyses data the driver just rebuilt into codex_cache_root
    % (e.g. revision_codex_2026); only fall back to source_cache_root for legacy
    % layouts. Reading source_cache_root FIRST risked loading a STALE Desktop copy
    % of "Flip_two - analyses data.mat" (caught 2026-06-01).
    flip_two_analysis = first_existing({
        fullfile(codex_cache_root, 'Flip_two - analyses data.mat')
        fullfile(codex_cache_root, 'Flip_two', 'analyses_data.mat')
    ...
    if isempty(flip_two_analysis)
        ...
        DCG_plot_Flip_two_interaction_2026_codex(flip_two_analysis, flip_two_output_dir, save_png);
        if verify_flip_two_interaction_figures && isfile(flip_two_verify_function)
            DCG_verify_Flip_two_figures_2026_codex(flip_two_output_dir);
        end
    end
end
