...
%
% Example:
%   DCG_calibrate_saved_fig_y_ranges(figures_root, datasets)
%
...
%   y_limits = DCG_calibrate_saved_fig_y_ranges(figures_root, datasets)
%   y_limits = DCG_calibrate_saved_fig_y_ranges(figures_root, datasets, figure_filenames)
%   y_limits = DCG_calibrate_saved_fig_y_ranges(figures_root, datasets, figure_filenames, save_png)
...
%        the common range, OVERWRITE the .fig via DCG_SAVEFIG_VISIBLE, and if
...
for f = 1 : numel(figure_filenames)
    ...
    for p = 1 : numel(existing_paths)
        ...
        dcg_savefig_visible(fig, fig_path);
        if save_png
            png_path = regexprep(fig_path, '\.fig$', '.png');
            try
                exportgraphics(fig, png_path, 'Resolution', 300);
            catch ME
                warning('DCG:exportGraphicsFailed', 'Could not export %s: %s', png_path, ME.message);
            end
        end
        close(fig);
    end

    fprintf('[DCG_calibrate_saved_fig_y_ranges] %s -> y=[%.6g %.6g] across %d figures.\n', ...
