...
%
%  Plotting entry point for the consolidated GNN Benchmark analyses.
...
if exist('GNNBenchmark_CONFIG', 'var') && isstruct(GNNBenchmark_CONFIG) && isfield(GNNBenchmark_CONFIG, 'dataset') && ~isempty(GNNBenchmark_CONFIG.dataset)
    dataset = GNNBenchmark_CONFIG.dataset;
...
elseif strcmp(dataset, 'Tissue_784')
    ...
    error('GNNBenchmark:unknownDataset', ['Unknown dataset "%s". Valid: v1_W, v1_UW, ', ...
...
% Optional caller override. Use a GNNBenchmark_CONFIG struct in the base workspace to
...
if exist('GNNBenchmark_CONFIG', 'var') && isstruct(GNNBenchmark_CONFIG)
    if isfield(GNNBenchmark_CONFIG, 'data_root'), data_root = GNNBenchmark_CONFIG.data_root; end
    if isfield(GNNBenchmark_CONFIG, 'cache_dir'), cache_dir = GNNBenchmark_CONFIG.cache_dir; end
    if isfield(GNNBenchmark_CONFIG, 'figures_root'), figures_root = GNNBenchmark_CONFIG.figures_root; end
    if isfield(GNNBenchmark_CONFIG, 'figures_subdir'), figures_subdir = GNNBenchmark_CONFIG.figures_subdir; end
    if isfield(GNNBenchmark_CONFIG, 'results_summary_filename'), results_summary_filename = GNNBenchmark_CONFIG.results_summary_filename; end
    if isfield(GNNBenchmark_CONFIG, 'figures_output_dir'), figures_output_dir = GNNBenchmark_CONFIG.figures_output_dir; end
    if isfield(GNNBenchmark_CONFIG, 'hex_analyses_filename'), hex_analyses_filename = GNNBenchmark_CONFIG.hex_analyses_filename; end
    if isfield(GNNBenchmark_CONFIG, 'hex_left_analyses_filename'), hex_left_analyses_filename = GNNBenchmark_CONFIG.hex_left_analyses_filename; end
    if isfield(GNNBenchmark_CONFIG, 'dataset_to_analyze'), dataset_to_analyze = GNNBenchmark_CONFIG.dataset_to_analyze; end
    if isfield(GNNBenchmark_CONFIG, 'seeds'), seeds = GNNBenchmark_CONFIG.seeds; end
    if isfield(GNNBenchmark_CONFIG, 'normalize_MAE'), normalize_MAE = GNNBenchmark_CONFIG.normalize_MAE; end
    if isfield(GNNBenchmark_CONFIG, 'use_log'), use_log = GNNBenchmark_CONFIG.use_log; end
    if isfield(GNNBenchmark_CONFIG, 'h_bins_for_quality_analysis'), h_bins_for_quality_analysis = GNNBenchmark_CONFIG.h_bins_for_quality_analysis; end
    if isfield(GNNBenchmark_CONFIG, 'h_bins_for_initial_histogram'), h_bins_for_initial_histogram = GNNBenchmark_CONFIG.h_bins_for_initial_histogram; end
    if isfield(GNNBenchmark_CONFIG, 'hex_paper_bins'), hex_paper_bins = GNNBenchmark_CONFIG.hex_paper_bins; end
    if isfield(GNNBenchmark_CONFIG, 'active_noise_paper_bins'), active_noise_paper_bins = GNNBenchmark_CONFIG.active_noise_paper_bins; end
    if isfield(GNNBenchmark_CONFIG, 'hex_paper_size_bin'), hex_paper_size_bin = GNNBenchmark_CONFIG.hex_paper_size_bin; end
    if isfield(GNNBenchmark_CONFIG, 'hex_paper_uncertainty'), hex_paper_uncertainty = GNNBenchmark_CONFIG.hex_paper_uncertainty; end
    if isfield(GNNBenchmark_CONFIG, 'hex_paper_split'), hex_paper_split = GNNBenchmark_CONFIG.hex_paper_split; end
    if isfield(GNNBenchmark_CONFIG, 'hex_save_separate_panels'), hex_save_separate_panels = GNNBenchmark_CONFIG.hex_save_separate_panels; end
    if isfield(GNNBenchmark_CONFIG, 'skip_cache_guard'), skip_cache_guard = GNNBenchmark_CONFIG.skip_cache_guard; end
    if isfield(GNNBenchmark_CONFIG, 'hex_left_size_bin'), hex_left_size_bin = GNNBenchmark_CONFIG.hex_left_size_bin; end
    if isfield(GNNBenchmark_CONFIG, 'hex_left_seed_col'), hex_left_seed_col = GNNBenchmark_CONFIG.hex_left_seed_col; end
    if isfield(GNNBenchmark_CONFIG, 'hex_left_model'), hex_left_model = GNNBenchmark_CONFIG.hex_left_model; end
    if isfield(GNNBenchmark_CONFIG, 'hex_left_task'), hex_left_task = GNNBenchmark_CONFIG.hex_left_task; end
    if isfield(GNNBenchmark_CONFIG, 'hex_left_splits'), hex_left_splits = GNNBenchmark_CONFIG.hex_left_splits; end
    if isfield(GNNBenchmark_CONFIG, 'plot_only_hexagonality_manuscript'), plot_only_hexagonality_manuscript = GNNBenchmark_CONFIG.plot_only_hexagonality_manuscript; end
    if isfield(GNNBenchmark_CONFIG, 'plot_hex_hop_diagnostics'), plot_hex_hop_diagnostics = GNNBenchmark_CONFIG.plot_hex_hop_diagnostics; end
    if isfield(GNNBenchmark_CONFIG, 'plot_fallback_analysis'), plot_fallback_analysis = GNNBenchmark_CONFIG.plot_fallback_analysis; end
    if isfield(GNNBenchmark_CONFIG, 'plot_hop_demos'), plot_hop_demos = GNNBenchmark_CONFIG.plot_hop_demos; end
    if isfield(GNNBenchmark_CONFIG, 'models_to_exclude'), models_to_exclude = GNNBenchmark_CONFIG.models_to_exclude; end
    if isfield(GNNBenchmark_CONFIG, 'figure_panel_size'), figure_panel_size = GNNBenchmark_CONFIG.figure_panel_size; end
    if isfield(GNNBenchmark_CONFIG, 'scatter_marker_size'), scatter_marker_size = GNNBenchmark_CONFIG.scatter_marker_size; end
    if isfield(GNNBenchmark_CONFIG, 'scores_to_show'), scores_to_show = GNNBenchmark_CONFIG.scores_to_show; end
    if isfield(GNNBenchmark_CONFIG, 'embed_color_percentiles'), embed_color_percentiles = GNNBenchmark_CONFIG.embed_color_percentiles; end
    if isfield(GNNBenchmark_CONFIG, 'embed_color_scale'), embed_color_scale = GNNBenchmark_CONFIG.embed_color_scale; end
    if isfield(GNNBenchmark_CONFIG, 'ratio_threshold_identity_function'), ratio_threshold_identity_function = GNNBenchmark_CONFIG.ratio_threshold_identity_function; end
    if isfield(GNNBenchmark_CONFIG, 'summary_dataset'), summary_dataset = GNNBenchmark_CONFIG.summary_dataset; end
    if isfield(GNNBenchmark_CONFIG, 'size_bins_to_keep'), size_bins_to_keep = GNNBenchmark_CONFIG.size_bins_to_keep; end
    if isfield(GNNBenchmark_CONFIG, 'colors'), colors = GNNBenchmark_CONFIG.colors; end
end

% Direct-script runs do not necessarily define GNNBenchmark_CONFIG. Keep the driver
...
if ~isfield(GNNBenchmark_CONFIG, 'embed_examples')
    GNNBenchmark_CONFIG.embed_examples = true;
end
if ~isfield(GNNBenchmark_CONFIG, 'embed_recompute')
    GNNBenchmark_CONFIG.embed_recompute = false;
end
if ~isfield(GNNBenchmark_CONFIG, 'embed_color_percentiles')
    GNNBenchmark_CONFIG.embed_color_percentiles = embed_color_percentiles;
end
if ~isfield(GNNBenchmark_CONFIG, 'embed_color_scale')
    GNNBenchmark_CONFIG.embed_color_scale = embed_color_scale;
end
if ~isfield(GNNBenchmark_CONFIG, 'embed_engine') || isempty(GNNBenchmark_CONFIG.embed_engine)
    GNNBenchmark_CONFIG.embed_engine = path_cfg.embed_engine;
end
if ~isfield(GNNBenchmark_CONFIG, 'embed_workdir') || isempty(GNNBenchmark_CONFIG.embed_workdir)
    GNNBenchmark_CONFIG.embed_workdir = path_cfg.embed_workdir;
end
if ~isfield(GNNBenchmark_CONFIG, 'embed_vt2d_std') || isempty(GNNBenchmark_CONFIG.embed_vt2d_std)
    GNNBenchmark_CONFIG.embed_vt2d_std = path_cfg.embed_vt2d_std;
end
if ~isfield(GNNBenchmark_CONFIG, 'embed_vt2d_rev') || isempty(GNNBenchmark_CONFIG.embed_vt2d_rev)
    GNNBenchmark_CONFIG.embed_vt2d_rev = path_cfg.embed_vt2d_rev;
end
...
fprintf('[GNNBenchmark_plot_results] figures output: %s\n', figures_output_dir);
setappdata(0, 'GNNBenchmark_CURRENT_DATASET_LABEL', sprintf('%s (%s)', plot_title, dataset));
...
if isfield(GNNBenchmark_CONFIG, 'make_extreme_task_composites_only') && ...
        isequal(GNNBenchmark_CONFIG.make_extreme_task_composites_only, true)
    GNNBenchmark_make_extreme_task_composites_internal(data_root, cache_dir, figures_root, ...
        dataset_to_analyze, h_bins_for_quality_analysis, GNNBenchmark_CONFIG);
    return;
end
...
fprintf('[GNNBenchmark_plot_results] dataset=%s | tasks=%s | n_cells=%d | multi_cohort=%d | multi_T1=%d | hex_overlay=%d\n', ...
...
% stamped 'source_manifest' (GNNBenchmark_source_manifest); recompute it from
...
% guard (no stamp). Override with GNNBenchmark_CONFIG.skip_cache_guard = true.
if ~skip_cache_guard
    ...
    if ~isfield(stamp_loaded, 'source_manifest')
        warning('GNNBenchmark:noCacheStamp', ['Summary "%s" has NO provenance stamp (built by an ' ...
            'older analyzer). Cannot verify it is current -- rebuild it ' ...
            '(rebuild_summaries=true) to be safe.'], results_summary_filename);
    elseif ~strcmp(stamp_loaded.source_manifest, GNNBenchmark_source_manifest(data_root))
        error('GNNBenchmark:staleCache', ['STALE SUMMARY -- refusing to plot.\n  %s\nwas built from a ' ...
            'DIFFERENT set of prediction files than currently sit in\n  %s\nRebuild it ' ...
            '(rebuild_summaries=true) before plotting. (Override: ' ...
            'GNNBenchmark_CONFIG.skip_cache_guard = true.)'], results_summary_filename, data_root);
    end
    if ~isfield(stamp_loaded, 'analysis_algorithm_version')
        error('GNNBenchmark:staleAnalysisAlgorithm', ['STALE SUMMARY -- refusing to plot.\n  %s\nwas built ' ...
...
if ~all(ismember(tasks, loaded_tasks))
    error('GNNBenchmark:taskMismatch', ...
...
if ~all(isfield(S.prediction_errors, tasks))
    error('GNNBenchmark:missingTaskField', ...
        'Loaded S.prediction_errors does not contain all selected tasks: [%s].', ...
        strjoin(tasks, ', '));
end
if ~isequal(tasks, loaded_tasks)
    fprintf('[GNNBenchmark_plot_results] Using selector tasks [%s]; loaded summary also contains [%s].\n', ...
...
if ~isempty(size_bins_to_keep)
    ...
    fprintf('[GNNBenchmark_plot_results] keeping size bin(s): %s (# cohorts: %s)\n', ...
...
if observed_max_hop > max_cell_dist
    fprintf('[GNNBenchmark_plot_results] max_cell_dist raised %d -> %d (data has deeper hops).\n', ...
...
if plot_only_hexagonality_manuscript
    ...
    if ~plot_hex_hop_diagnostics
        fprintf('[GNNBenchmark_plot_results] plot_only_hexagonality_manuscript=true -- generated manuscript hexagonality panels only.\n');
        return;
    end
    fprintf('[GNNBenchmark_plot_results] plot_only_hexagonality_manuscript=true with plot_hex_hop_diagnostics=true -- continuing to hex-only hop diagnostics.\n');
end
...
GNNBenchmark_savefig_visible(fullfile(figures_output_dir, 'hop demos.fig'));
else
    fprintf('[GNNBenchmark_plot_results] skipping hop demos for this dataset/task selection.\n');
end
...
if do_scatter_examples
    ...
    try
        ...
        warning('GNNBenchmark:scatterFailed', ...
            'plot_scatter_plot_examples failed: %s', ME.message);
    end
else
    fprintf('[GNNBenchmark_plot_results] skipping plot_scatter_plot_examples for this dataset/task selection.\n');
...
% only for missing cache entries). Set GNNBenchmark_CONFIG.embed_examples = false to skip.
...
% default ON: embed unless GNNBenchmark_CONFIG.embed_examples is explicitly set to false.
% GNNBenchmark_CONFIG.embed_recompute defaults false, so existing out_<sim_id> cache
% files are reused and only missing relaxations are calculated/saved.
do_embed = ~(exist('GNNBenchmark_CONFIG', 'var') && isstruct(GNNBenchmark_CONFIG) && ...
    isfield(GNNBenchmark_CONFIG, 'embed_examples') && isequal(GNNBenchmark_CONFIG.embed_examples, false));
...
if do_embed && ~strcmp(dataset, 'hex')
    ...
    try
        plot_embedding_examples(scores_to_show, tasks, MAE_individuals, S, figures_output_dir, all_models, dataset_to_analyze, dataset, data_root, GNNBenchmark_CONFIG);
    catch ME
        new_figs_e = setdiff(findall(groot, 'Type', 'figure'), pre_figs_e);
        if ~isempty(new_figs_e), close(new_figs_e); end
        warning('GNNBenchmark:embedFailed', 'plot_embedding_examples failed: %s', ME.message);
    end
end
...
if multi_cohort
    ...
    if is_hex_overlay
        fprintf('[GNNBenchmark_plot_results] NOTE on the MAE-vs-size figures: this is the hex run, so x-axis position 4 (cohort size 8) is the HEX tissue, NOT v1 size-8. Standard v1 cohorts are at positions 1, 2, 3, 5, 6.\n');
    end
else
    fprintf('[GNNBenchmark_plot_results] single-cohort dataset -- skipping MAE-vs-dataset-size plots.\n');
end
...
% GNNBenchmark_plot_Flip_two_interaction using h_min=min(d1,d2), inter-flip
...
if ~multi_T1
    ...
    fprintf('[GNNBenchmark_plot_results] multi-T1 dataset -- skipping generic single-root MAE-vs-distance plot; Flip_two-specific nearest/inter-flip diagnostics are produced by GNNBenchmark_plot_Flip_two_interaction.\n');
end
...
fprintf('[GNNBenchmark_plot_results] skipping generic MAE-vs-hexagonality plots; manuscript hexagonality uses dataset=''hex'' only.\n');
...
if ~manuscript_hex_panels_already_done && strcmp(dataset, 'hex') && isfile(hex_analyses_filename)
    try
        ...
        warning('GNNBenchmark:hexPaperPanelsFailed', ...
...
if plot_fallback_analysis
    ...
    try
        ...
        new_figs = setdiff(findall(groot, 'Type', 'figure'), pre_figs);
        if ~isempty(new_figs), close(new_figs); end
        warning('GNNBenchmark:fallbackFailed', ...
            'plot_PPGN_fallback errored (%s focus): %s', fallback_focus_model, ME.message);
    end
else
    fprintf('[GNNBenchmark_plot_results] skipping fallback analysis by configuration.\n');
end
...
function GNNBenchmark_savefig_visible(varargin)
...
% GNNBenchmark_savefig_visible  Save a figure as .fig with a dataset-tagged window name.
%
% PURPOSE
...
%
% INPUTS (variadic)
%   GNNBenchmark_savefig_visible(filename)            : saves the current figure (gcf).
%   GNNBenchmark_savefig_visible(fig_handle, filename): saves the given figure handle.
%
% OUTPUTS
...
%   GNNBenchmark_CURRENT_DATASET_LABEL on root (0) is set and non-empty, set the figure
...
if isappdata(0, 'GNNBenchmark_CURRENT_DATASET_LABEL')
    dataset_label = getappdata(0, 'GNNBenchmark_CURRENT_DATASET_LABEL');
else
    dataset_label = '';
end
...
if isfile(target)
    delete(target);
    fprintf('[GNNBenchmark_plot_results] removed stale plot file: %s\n', target);
end
...
for i = 1 : numel(stale_files)
    ...
    delete(target);
    fprintf('[GNNBenchmark_plot_results] removed stale scatter example file: %s\n', target);
end
...
for p = 1 : numel(stale_patterns)
    ...
    for i = 1 : numel(stale_files)
        ...
        if isfile(target)
            delete(target);
            fprintf('[GNNBenchmark_plot_results] removed stale fallback output: %s\n', target);
        end
    end
end
...
for p = 1 : numel(stale_patterns)
    ...
    for i = 1 : numel(stale_files)
        ...
        if isfile(target)
            delete(target);
            fprintf('[GNNBenchmark_plot_results] removed stale non-hex-manuscript output from hex folder: %s\n', target);
        end
    end
end
...
if fid < 0
    warning('GNNBenchmark:normalizationAssumptionsWriteFailed', 'Could not write normalization assumptions file.');
    return;
end
...
if ~any(drop_mask)
    fprintf('[GNNBenchmark_plot_results] models_to_exclude requested [%s], but none were present.\n', ...
        strjoin(models_to_exclude, ', '));
    return;
end

keep_mask = ~drop_mask;
fprintf('[GNNBenchmark_plot_results] excluding model columns from plots: %s\n', ...
...
if ~any(focus_idx_mask)
    error('GNNBenchmark:focusModelNotFound', 'focus_model "%s" not in all_models. Available: %s', focus_model, strjoin(all_models, ', '));
end
...
if ~any(focus_rows)
    ...
    if best_count > 0
        ...
        warning('GNNBenchmark:noFallbackExamples', 'No fallback examples found for any model.');
        return;
    end
end
...
GNNBenchmark_savefig_visible(fig_hist, fullfile(figures_output_dir, [focus_model, ' fallback hop distribution.fig']));
...
GNNBenchmark_savefig_visible(fig_cp, fullfile(figures_output_dir, [focus_model, ' fallback example change point.fig']));
...
if isempty(before_cells) || isempty(after_cells)
    warning('GNNBenchmark:fallbackExampleEmpty', 'Selected fallback example has an empty before/after segment.');
    return;
end
...
GNNBenchmark_savefig_visible(fig_scatter, fullfile(figures_output_dir, [focus_model, ' fallback example scatter plot.fig']));
...
%   shows n, mean and SD; square axes; save via GNNBenchmark_savefig_visible.
...
GNNBenchmark_savefig_visible(fullfile(figures_output_dir, 'Hexagonality distribution.fig'));
...
if isempty(model_fields)
    warning('GNNBenchmark:hexCacheNoModelsAfterExclusion', ...
...
if save_separate_panels
    ...
    GNNBenchmark_savefig_visible(fig_a, fullfile(figures_output_dir, 'Manuscript hexagonality vs active noise.fig'));

    fig_b = figure('Position', [760 300 430 390]);
    ax_b = axes(fig_b);
    draw_manuscript_hex_error(ax_b, x_hex, y_hex, e_hex, colors, all_models_for_plot);
    GNNBenchmark_savefig_visible(fig_b, fullfile(figures_output_dir, 'Manuscript identity-normalized error vs hexagonality.fig'));
end
...
ax_b = nexttile(tl, 2);
draw_manuscript_hex_error(ax_b, x_hex, y_hex, e_hex, colors, all_models_for_plot);
GNNBenchmark_savefig_visible(fig, fullfile(figures_output_dir, 'Manuscript hexagonality panels.fig'));
...
%
% DECISIONS / EDGE CASES
%   Errors (GNNBenchmark:testOnlyRequired) if split_name is not 'test'; errors
%   (GNNBenchmark:hexSplitMissing) if the split is absent from the cache.
...
if ~isfield(I.(model_field).(task).inds{subset_idx}, split_name)
    error('GNNBenchmark:hexSplitMissing', 'Split "%s" not found in the hex analyses cache.', split_name);
end
...
if nargin < 8 || isempty(size_bin)
    ...
    if isempty(size_bin)
        warning('GNNBenchmark:hexPaperNoSize', 'No populated lengths_to_lengths size bins; skipping manuscript hexagonality panels.');
        return;
    end
end
...
switch lower(char(uncertainty_mode))
    ...
    case 'sd'
        e = sd;
    otherwise
        warning('GNNBenchmark:unknownUncertainty', 'Unknown uncertainty "%s"; using SD.', char(uncertainty_mode));
        e = sd;
end
...
for t = 1 : length(tasks)
    ...
    if isempty(scores)
        warning('GNNBenchmark:scatterNoScores', 'No finite graph/model MAE scores available for task %s.', task);
        continue;
    end
    ...
    for i = 1 : length(scores_to_show)
        ...
        if ~any(valid_curr_hops)
            warning('GNNBenchmark:scatterMissingModelColumn', ...
    ...
    GNNBenchmark_savefig_visible(fid, fullfile(figures_output_dir, ['Scatter plot examples (', task, ').fig']));
end
...
GNNBenchmark_savefig_visible(fid, fullfile(figures_output_dir, 'MAE vs dataset size (normalized log scale).fig'));
...
GNNBenchmark_savefig_visible(fullfile(figures_output_dir, 'MAE vs traverse dist.fig'));
...
GNNBenchmark_savefig_visible(fullfile(figures_output_dir, 'MAE vs hexagonality.fig'));
...
% ============================================================================
function plot_embedding_examples(scores_to_show, tasks, MAE_individuals, S, figures_output_dir, all_models, dataset_to_analyze, dataset, data_root, GNNBenchmark_CONFIG)
% plot_embedding_examples  Render the plot embedding examples panel or plotting primitive.
% Inputs: scores_to_show, tasks, MAE_individuals, S, figures_output_dir, all_models, dataset_to_analyze, dataset, data_root, GNNBenchmark_CONFIG
% Outputs: none; performs side effects or updates the caller workflow.
...
%   data_root          : default prediction-file root (overridable via GNNBenchmark_CONFIG).
%   GNNBenchmark_CONFIG         : optional config struct (engine path, work/vt2d roots,
...
%
% EXTERNAL ENGINE
%   spring_embed.exe (path from GNNBenchmark_CONFIG.embed_engine). Each variant
...
engine    = emb_getcfg(GNNBenchmark_CONFIG, 'embed_engine',   '');
workdir   = emb_getcfg(GNNBenchmark_CONFIG, 'embed_workdir',  fullfile(tempdir, 'GNNBenchmark_springs_embed'));
stdRoot   = emb_getcfg(GNNBenchmark_CONFIG, 'embed_vt2d_std', '');
revRoot   = emb_getcfg(GNNBenchmark_CONFIG, 'embed_vt2d_rev', '');
recompute = emb_getcfg(GNNBenchmark_CONFIG, 'embed_recompute', false);
colorPct  = emb_getcfg(GNNBenchmark_CONFIG, 'embed_color_percentiles', [0.5 99]);
shearVt2d = emb_getcfg(GNNBenchmark_CONFIG, 'embed_shear_affine_vt2d', true);
colorScale = lower(string(emb_getcfg(GNNBenchmark_CONFIG, 'embed_color_scale', 'linear')));
if ~ismember(colorScale, ["log", "linear"])
    warning('GNNBenchmark:embedColorScale', 'unknown embed_color_scale=%s; using linear.', colorScale);
    colorScale = "linear";
end
useLogColor = colorScale == "log";
saveOverlay = emb_getcfg(GNNBenchmark_CONFIG, 'embed_save_overlay', true);
savePerEdge = emb_getcfg(GNNBenchmark_CONFIG, 'embed_save_per_edge', true);
...
% emb_dataset_prefix). Override the roots via GNNBenchmark_CONFIG if the data moves.
% When predRoot is a consolidated snapshot the names/splits differ; the
% GNNBenchmark_consolidated_paths resolver handles both (pred + inds) below.
predRoot = emb_getcfg(GNNBenchmark_CONFIG, 'embed_pred_root', data_root);
indsRoot = emb_getcfg(GNNBenchmark_CONFIG, 'embed_inds_root', fullfile(data_root, 'inds'));
emb_consolidated = GNNBenchmark_consolidated_paths('is_consolidated', predRoot);
...
for t = 1 : length(tasks)
    ...
    if saveOverlay
        ...
        try savefig(figA, fullfile(figures_output_dir, ['Embedding examples overlay (', tk, ').fig'])); catch ME, warning('GNNBenchmark:embedSavefigA', 'embed savefig A: %s', ME.message); end
    ...
    % Configure with GNNBenchmark_CONFIG.embed_color_percentiles; default [0.5 99], use
    ...
    % same units. GNNBenchmark_CONFIG.embed_color_scale='log' is retained only as an
    ...
    if savePerEdge
        ...
        try savefig(figB, fullfile(figures_output_dir, ['Embedding examples per-edge (', tk, ').fig'])); catch ME, warning('GNNBenchmark:embedSavefigB', 'embed savefig B: %s', ME.message); end
...
%
% PURPOSE
%   Tiny accessor used to pull optional embedding settings out of GNNBenchmark_CONFIG
%   with a fallback.
%
% INPUTS
...
try
    ...
    warning('GNNBenchmark:embedShearVt2d', ...
...
p = colorPct(:).';
if numel(p) ~= 2
    error('GNNBenchmark:embedColorPercentiles', 'embed_color_percentiles must be [] or [low high].');
end
p = max(0, min(100, p));
...
function output_paths = GNNBenchmark_make_extreme_task_composites_internal(data_root, cache_dir, figures_root, dataset_to_analyze, h_bins_for_quality_analysis, GNNBenchmark_CONFIG)
% GNNBenchmark_make_extreme_task_composites_internal  Implement GNN Benchmark make extreme task composites internal for this MATLAB workflow.
% Inputs: data_root, cache_dir, figures_root, dataset_to_analyze, h_bins_for_quality_analysis, GNNBenchmark_CONFIG
% Outputs: output_paths
% GNNBenchmark_make_extreme_task_composites_internal  Temporary 5x3 revision composites.
%
...
for i = 1 : numel(all_dataset_keys)
    ...
    try
        ...
        warning('GNNBenchmark:extremeLoadFailed', 'Could not load %s for extreme composites: %s', key, ME.message);
    end
end
...
output_paths.condition_log2_mae = extreme_plot_family_condition_grid(families, model_rows, bundles, ...
    'log2mae', 'log2(mean graph MAE)', output_dir, save_png);
if isequal(extreme_cfg(GNNBenchmark_CONFIG, 'extreme_skip_embedding', false), true)
    output_paths.embedding_overlay = '';
    output_paths.embedding_per_edge = '';
    fprintf('[GNNBenchmark_make_extreme_task_composites_internal] extreme_skip_embedding=true, skipped embedding grids.\n');
else
    [output_paths.embedding_overlay, output_paths.embedding_per_edge] = extreme_plot_embedding_grids( ...
        model_rows, bundles, data_root, dataset_to_analyze, output_dir, save_png, GNNBenchmark_CONFIG);
end

extreme_write_assumptions(output_dir, families, model_rows, output_paths);
fprintf('[GNNBenchmark_make_extreme_task_composites_internal] wrote composites under: %s\n', output_dir);
end
...
function [overlay_path, per_edge_path] = extreme_plot_embedding_grids(model_rows, bundles, data_root, dataset_to_analyze, output_dir, save_png, GNNBenchmark_CONFIG)
% extreme_plot_embedding_grids  Handle cross-condition revision-figure plot embedding grids logic.
% Inputs: model_rows, bundles, data_root, dataset_to_analyze, output_dir, save_png, GNNBenchmark_CONFIG
...
[sel_grid, geom_grid] = extreme_collect_embedding_geometry(model_rows, extreme_datasets, bundles, ...
    data_root, dataset_to_analyze, GNNBenchmark_CONFIG);
...
useLogColor = strcmpi(extreme_cfg(GNNBenchmark_CONFIG, 'embed_color_scale', 'linear'), 'log');
colorPct = extreme_cfg(GNNBenchmark_CONFIG, 'embed_color_percentiles', []);
for r = 1 : numel(model_rows)
    for c = 1 : numel(extreme_datasets)
...
function [sel_grid, geom_grid] = extreme_collect_embedding_geometry(model_rows, dataset_keys, bundles, data_root, dataset_to_analyze, GNNBenchmark_CONFIG)
% extreme_collect_embedding_geometry  Handle cross-condition revision-figure collect embedding geometry logic.
% Inputs: model_rows, dataset_keys, bundles, data_root, dataset_to_analyze, GNNBenchmark_CONFIG
...
engine = extreme_cfg(GNNBenchmark_CONFIG, 'embed_engine', '');
workdir = extreme_cfg(GNNBenchmark_CONFIG, 'embed_workdir', fullfile(tempdir, 'GNNBenchmark_springs_embed'));
stdRoot = extreme_cfg(GNNBenchmark_CONFIG, 'embed_vt2d_std', '');
revRoot = extreme_cfg(GNNBenchmark_CONFIG, 'embed_vt2d_rev', '');
predRoot = extreme_cfg(GNNBenchmark_CONFIG, 'embed_pred_root', data_root);
indsRoot = extreme_cfg(GNNBenchmark_CONFIG, 'embed_inds_root', fullfile(data_root, 'inds'));
recompute = extreme_cfg(GNNBenchmark_CONFIG, 'embed_recompute', false);
shearVt2d = extreme_cfg(GNNBenchmark_CONFIG, 'embed_shear_affine_vt2d', true);
emb_consolidated = GNNBenchmark_consolidated_paths('is_consolidated', predRoot);
        ...
        for v = 1 : size(variants, 1)
            ...
            if recompute || exist(marker, 'file') ~= 2
                ...
                try
                    ...
                    warning('GNNBenchmark:extremeEmbedWrite', 'Could not write embedding input for %s/%s: %s', sel.cfgkey, tag, ME.message);
                end
            end
        end
        sel_grid(r,c) = sel;
...
try
    ...
    warning('GNNBenchmark:extremeEmbedGeometry', 'Could not load geometry for %s: %s', sel.cfgkey, ME.message);
end
end
...
fprintf('[GNNBenchmark extreme composites] saved %s\n', fig_path);
end
