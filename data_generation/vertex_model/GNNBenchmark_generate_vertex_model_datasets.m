function report = GNNBenchmark_generate_vertex_model_datasets(varargin)
% GNNBenchmark_generate_vertex_model_datasets  Implement GNN Benchmark generate vertex model datasets for this MATLAB workflow.
% Inputs: varargin
% Outputs: report
%GNNBenchmark_GENERATE_VERTEX_MODEL_DATASETS  Generate manuscript vertex-model graphs.
%
%   REPORT = GNNBenchmark_GENERATE_VERTEX_MODEL_DATASETS() builds the vertex-model
...
%
%   REPORT = GNNBenchmark_GENERATE_VERTEX_MODEL_DATASETS('mode','publication') runs the
...
specs = GNNBenchmark_vertex_model_publication_manifest(opts.mode);
...
for s = 1:numel(specs)
    ...
    if opts.counterfactual && should_write_counterfactual(spec.key, opts.counterfactual_datasets)
        ...
        cf_assembly = GNNBenchmark_assemble_vertex_model_graphs(raw_output_dir, spec.rows, ...
...
% local_paths  Implement local paths for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
% Inputs: none.
% Outputs: paths
...
% build_simulator  Implement build simulator for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
...
% run_initial_phase  Implement run initial phase for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
...
% run_graph_phase  Implement run graph phase for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
% Inputs: exe_path, run_root, rows, opts
% Outputs: none; performs side effects or updates the caller workflow.
...
% run_commands  Implement run commands for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
...
% make_run_command  Implement make run command for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
...
% copy_split_manifests  Implement copy split manifests for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
...
% copy_default_split  Implement copy default split for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
...
% expected_raw_graph_name  Implement expected raw graph name for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
% Inputs: row
% Outputs: name
...
% table_number  Implement table number for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
% Inputs: row, name
% Outputs: x
v = row.(name);
...
% c_g_format  Implement c g format for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
% Inputs: x
% Outputs: s
s = sprintf('%.6g', x);
end
...
% ensure_dir  Implement ensure dir for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
% Inputs: d
% Outputs: none; performs side effects or updates the caller workflow.
if ~isfolder(d)
    mkdir(d);
...
% executable_name  Implement executable name for data_generation/vertex_model/GNNBenchmark_generate_vertex_model_datasets.m.
% Inputs: none.
% Outputs: name
if ispc
