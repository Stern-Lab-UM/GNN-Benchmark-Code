...
void outGraph(){
    ...
    FILE *file2;
    snprintf(filename1, sizeof(char) * 100, "./output/graph_%d_%d_%d_%d_%d_%g.txt", (int)Nc, packageID, SigI, T1EdgeID_1, T1EdgeID_2, shearFactor);
    file2 = fopen(filename1, "wt");

    for(int i=1; i<=Ne; i++) if(exist[2][i]==1){
        int c1=e_cells[i][1];
        int c2=e_cells[i][2];
        if(i==T1EdgeID_1 || i==T1EdgeID_2) fprintf(file2, "%d  %d  1  %.8g  %.8g\n", c1, c2, e_length_initial[i], edge_length(i));
        else fprintf(file2, "%d  %d  0  %.8g  %.8g\n", c1, c2, e_length_initial[i], edge_length(i));
    }
...
void out_Vertissue2D(){
    ...
    char filename2[100];
    snprintf(filename2, sizeof(char) * 100, "./output/final_%d_%g_%d_%d.vt2d", (int)Nc, kA, packageID, SigI);
