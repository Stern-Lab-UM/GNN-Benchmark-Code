function report = GNNBenchmark_assemble_vertex_model_graphs(raw_output_dir, rows, output_dir, dataset_key, varargin)
% GNNBenchmark_assemble_vertex_model_graphs  Implement GNN Benchmark assemble vertex model graphs for this MATLAB workflow.
% Inputs: raw_output_dir, rows, output_dir, dataset_key, varargin
% Outputs: report
%GNNBenchmark_ASSEMBLE_VERTEX_MODEL_GRAPHS  Convert raw vertex-model graphs to ML files.
%
%   REPORT = GNNBenchmark_ASSEMBLE_VERTEX_MODEL_GRAPHS(RAW_OUTPUT_DIR, ROWS,
...
p = inputParser;
p.FunctionName = 'GNNBenchmark_assemble_vertex_model_graphs';
...
% assemble_one_graph  Implement assemble one graph for data_generation/vertex_model/GNNBenchmark_assemble_vertex_model_graphs.m.
...
% infer_new_edge_cells  Implement infer new edge cells for data_generation/vertex_model/GNNBenchmark_assemble_vertex_model_graphs.m.
...
% resolve_raw_graph_path  Implement resolve raw graph path for data_generation/vertex_model/GNNBenchmark_assemble_vertex_model_graphs.m.
...
% expected_raw_graph_name  Implement expected raw graph name for data_generation/vertex_model/GNNBenchmark_assemble_vertex_model_graphs.m.
% Inputs: row
% Outputs: name
n_cells = table_number(row, 'n_cells');
...
% table_number  Implement table number for data_generation/vertex_model/GNNBenchmark_assemble_vertex_model_graphs.m.
% Inputs: row, name
% Outputs: x
v = row.(name);
...
% c_g_format  Implement c g format for data_generation/vertex_model/GNNBenchmark_assemble_vertex_model_graphs.m.
% Inputs: x
% Outputs: s
s = sprintf('%.6g', x);
end
